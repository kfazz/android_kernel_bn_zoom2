...
#endif /* AS3676_DLS_ALS */

/* Setting for AS3676_REG_curr12_control: 00b=off, 01b=on, 10b=PWM, 11b=pattern */
#define AS3676_CURR126_CONTROL_OFF		0x00
#define AS3676_CURR126_CONTROL_ON		0x01
#define AS3676_CURR126_CONTROL_PWM		0x02
#define AS3676_CURR126_CONTROL_PATTERN		0x03
...
#define AS3676_DIM_LOCK()	down(&(data)->dimming_in_progress_sema)
#define AS3676_DIM_UNLOCK()	up(&(data)->dimming_in_progress_sema)
#define AS3676_DIM_IS_LOCKED() (data->dimming_in_progress == 1)
...
struct as3676_data {
	...
	struct semaphore dimming_in_progress_sema;
	bool dimming_in_progress;
	u32 shutdown_timer_msec;
	int pattern_running;
	int num_leds;
	int ldo_count;
...
static void pwronkey_work(struct work_struct *work);
static void pwronkey_event(struct input_handle *handle,
	unsigned int type, unsigned int code, int value);
static int pwronkey_connect(struct input_handler *handler,
	struct input_dev *dev, const struct input_device_id *id);
static void pwronkey_disconnect(struct input_handle *handle);
...
static bool do_shutdown_process = true;

static const struct input_device_id pwronkey_ids[] = {
	{
		/* Connect only to input registered with KEY_POWER */
...
MODULE_DEVICE_TABLE(input, pwronkey_ids);

static struct input_handler pwronkey_handler = {
	.event = pwronkey_event,
	.connect = pwronkey_connect,
	.disconnect = pwronkey_disconnect,
	.name = "pwronkey",
	.id_table =	pwronkey_ids,
};

static void pwronkey_work(struct work_struct *work)
{
	int i;
	struct as3676_data * const data =
	    container_of(work, struct as3676_data, shutdown_work.work);

	do_shutdown_process = false;

	AS3676_LOCK();
	/* Reset all registers to default values */
	for (i = 0; i < ARRAY_SIZE(as3676_default_data.regs); i++) {
		if (as3676_default_data.regs[i].name) {
			i2c_smbus_write_byte_data(data->client,
			i, as3676_default_data.regs[i].value);
		}
	}

	/* Do not do AS3676_UNLOCK Here. Lock-up all register access then poweroff kernel. */
	kernel_power_off();
}
	...
	unsigned int type, unsigned int code, int value)
{
	if (KEY_POWER == code) {
		struct as3676_data * const data =
			(struct as3676_data *)handle->private;

		if (1 == value) {
			schedule_delayed_work(&(data->shutdown_work),
				      msecs_to_jiffies(data->shutdown_timer_msec));
		} else if ((0 == value) && do_shutdown_process) {
...
static int pwronkey_connect(struct input_handler *handler,
	struct input_dev *dev, const struct input_device_id *id)
{
	struct as3676_data * const data = (struct as3676_data *)handler->private;
	...
	INIT_DELAYED_WORK(&data->shutdown_work, pwronkey_work);
	...
	pr_debug("as3676: Connected device: %s (%s at %s)\n",
	       dev_name(&dev->dev),
	       dev->name ?: "unknown",
	       dev->phys ?: "unknown");
...
static void pwronkey_disconnect(struct input_handle *handle)
{
	pr_debug("as3676: Disconnected device: %s\n",
...
static int as3676_suspend(struct device *dev)
{
	...
	/* Acquire the dimming lock in case dimming is still in progress */
	AS3676_DIM_LOCK();
...
#else
static int as3676_i2c_suspend(struct i2c_client *client, pm_message_t msg)
{
	return as3676_suspend(&client->dev);
}

static int as3676_i2c_resume(struct i2c_client *client)
{
	return as3676_resume(&client->dev);
}
#endif

static struct dev_pm_ops as3676_pm = {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 33)
...
#ifdef CONFIG_PM
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 33)
	.suspend = as3676_i2c_suspend,
	.resume = as3676_i2c_resume,
#endif
#endif
};
...
static void as3676_switch_led(struct as3676_data *data,
			      struct as3676_led *led, int mode)
{
	...
	case 0: /* LED OFF */
		...
		AS3676_MODIFY_REG(led->mode_reg, 0x03 << led->mode_shift,
			AS3676_CURR126_CONTROL_OFF);
	...
	case 1:{ /* LED ON */
			...
			const u8 on_val = (led->use_pattern ?
				AS3676_CURR126_CONTROL_PATTERN :
				AS3676_CURR126_CONTROL_ON);
...
static void as3676_set_led_brightness(struct led_classdev *led_cdev,
				      enum led_brightness value)
{
	struct device * const dev = led_cdev->dev->parent;
	struct as3676_data * const data = dev_get_drvdata(dev);
	struct as3676_led * const led = ldev_to_led(led_cdev);

	AS3676_DIM_LOCK();
	// Control both CURR1/CURR2 as one single unit for Gossamer/Condor
#ifdef CONFIG_MACH_OMAP3621_GOSSAMER_EVT1C
	as3676_set_brightness(data, &data->leds[0], value);
	as3676_set_brightness(data, &data->leds[1], value);
#else
	as3676_set_brightness(data, led, value);
#endif
	AS3676_DIM_UNLOCK();

	if ((led->dim_brightness != -1) && AS3676_DIM_IS_LOCKED()) {
		dev_dbg(dev, "LED %s is currently being dimmed, changing"
...
static void as3676_set_brightness(struct as3676_data *data,
				  ...
				  enum led_brightness value)
{
	const u8 prev_value = AS3676_READ_REG(led->reg);
					   ...
					   const char *buf, size_t size)
{
	...
	if (dim_brightness > 255 || dim_brightness < -1)
		return -EINVAL;
	if (AS3676_DIM_IS_LOCKED()) {
		dev_dbg(dev, "Cannot change dimming parameters while dimming"
			 " is in progress\n");
		/* Fall-thru */
	}
					...
					char *buf)
{
	...
	if (AS3676_READ_REG(AS3676_REG_Pattern_control) & 1) {
		...
		MSNPRINTF("use_color == 0: 32 bit pattern " "data %#010x\n", p);
		for (i = 0; i < 32; i++) {
			if (p & (1 << i))
				MSNPRINTF("-");
			else
					 ...
					 const char *buf, size_t size)
{
	...
	AS3676_LOCK();
	AS3676_WRITE_REG(AS3676_REG_Pattern_data0, pdata & 0xff);
	pdata >>= 8;
	AS3676_WRITE_REG(AS3676_REG_Pattern_data1, pdata & 0xff);
	pdata >>= 8;
	AS3676_WRITE_REG(AS3676_REG_Pattern_data2, pdata & 0xff);
	pdata >>= 8;
...
static ssize_t as3676_als_filter_up_speed_mHz_show(struct device *dev, struct device_attribute
						   *attr, char *buf)
{
	return as3676_als_filter_speed_mHz_show(dev, buf, 0);
}

static ssize_t as3676_als_filter_up_speed_mHz_store(struct device *dev, struct device_attribute
						    *attr, const char *buf,
						    size_t size)
{
...
static ssize_t as3676_als_filter_down_speed_mHz_show(struct device *dev, struct device_attribute
						     *attr, char *buf)
{
	return as3676_als_filter_speed_mHz_show(dev, buf, 4);
}

static ssize_t as3676_als_filter_down_speed_mHz_store(struct device *dev, struct device_attribute
						      *attr, const char *buf,
						      size_t size)
{
						...
						const char *buf, size_t size)
{
	...
	if (use_softdim && AS3676_DIM_IS_LOCKED()) {
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	MSNPRINTF("%d,%d,%d,%d,%d,%d"
		  ...
		  y0, x1, k1, x2, k2, y3, y0, x1, k1, x2, k2, y3);
...
static const u32 dim_speed2time[8] = { 1000, 1900, 500, 950, 100, 190, 50, 95 };
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	u32 dim_time = dim_speed2time[dim_speed];
				      ...
				      const char *buf, size_t size)
{
	...
	if (dim_start && AS3676_DIM_IS_LOCKED()) {
		dev_dbg(dev, "Dimming in progress\n");
		/* Fall-thru */
	}
	...
	if (unlikely(err)) {
		dev_warn(dev, "Inconsistent cross dimming, "
			 ...
			 "from the same value or down to the same " "value!\n");
		AS3676_DIM_UNLOCK();
		goto exit_clr;
	}
				      ...
				      const char *buf, size_t size)
{
	...
	AS3676_MODIFY_REG(led->mode_reg + 0x93,
			  3 << led->mode_shift, als_group << led->mode_shift);
	AS3676_UNLOCK();
	return strnlen(buf, PAGE_SIZE);
}
			    ...
			    struct as3676_platform_data *pdata)
{
	...
	AS3676_WRITE_REG(AS3676_REG_DCDC_control1,
		(pdata->step_up_frequ) |
		(pdata->step_up_vtuning << 3) |
		(0x2 << 1) /* bit[2:1]: CURR2 feedback enabled */
		);

#ifdef AS3678XM
	AS3676_MODIFY_REG(AS3676_REG_Audio_Input, 0x7, pdata->audio_gain);
	...
	if ((pdata->audio_source == 0)
	    && (AS3676_MAX_NUM_LEDS == data->num_leds))
		data->num_leds--;
#endif /* AS3687XM */
...
{
	struct as3676_data *data;
	struct as3676_platform_data * const as3676_pdata = client->dev.platform_data;
	int id1, id2, i;
	int err = 0;

	if (unlikely(!as3676_pdata))
		return -EIO;
	...
	*data = as3676_default_data;
	...
	/* Must use semaphore here for dimming inter-thread synchronization */
	 sema_init(&data->dimming_in_progress_sema,1);
	...
	if (as3676_pdata->pwronkey_shutdown_msec > 0) {
		int ret;
		data->shutdown_timer_msec = as3676_pdata->pwronkey_shutdown_msec;
		pwronkey_handler.private = (void *)data;
		ret = input_register_handler(&pwronkey_handler);
		if (0 != ret) {
			err = ret;
			dev_err(&client->dev, "cannot register input handler");
			goto exit;
		}
	}

	dev_info(&client->dev, "AS3676 compatible chip ids: %x %x\n", id1, id2);
	/* all voltages on */
	data->client = client;
	...
	pr_debug("LED ---------------- AS3676 driver detected\n");

	AS3676_WRITE_REG(AS3676_REG_Control, 0x00);
	AS3676_WRITE_REG(AS3676_REG_CP_control, 0x12);
	AS3676_WRITE_REG(AS3676_REG_DCDC_control2, 0xB6);
...
exit:
	if (err) {
		kfree(data);
		dev_err(&client->dev, "exit with err: %x\n", err);
...
static int as3676_remove(struct i2c_client *client)
{
	struct as3676_data * const data = i2c_get_clientdata(client);
	int i;

	if (data->shutdown_timer_msec > 0)
		input_unregister_handler(&pwronkey_handler);
