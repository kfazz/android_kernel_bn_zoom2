...
extern int omap3ep_user_update(struct fb_info *info, struct omap3epfb_update_area *p);
...
///#TODO
		...
		case OMAP3EPFB_IO_REQ_AREA_UPDATE:
			...
			if (copy_from_user(&p.update_area, (void __user *)arg,
				...
				if (!omap3ep_user_update(info, &p.update_area))
					break;
...
static int __init omap3epfb_init(void)
{
	...
	if (!ret) {
		...
		if (ret) {
			...
			printk("omap3epfb_init failed \n");
		}
	}
		printk("omap3epfb_init succeeded \n");
	return ret;
}
