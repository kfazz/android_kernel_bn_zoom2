...
#if defined(CONFIG_KTIME_SCALAR)
#include <asm/div64.h>
#endif
...
static inline u32 omap3ep_ktime_to_ms(void)
{
    ktime_t tm = ktime_get();
#if defined(CONFIG_KTIME_SCALAR)
    return do_div(tm.tv64,1000000);
#else
...
int omap3ep_user_update(struct fb_info *info, struct omap3epfb_update_area *p)
{
	struct omap3epfb_par *par = info->par;
	int percent = 0;
	u32 start_time = 0;
